...
function getCommonDetails(details, settings) {
    var _a, _b, _c;
    const interfaceOrTypeName = joiUtils_1.getInterfaceOrTypeName(settings, details);
...
function parseSchema(details, settings, useLabels = true, ignoreLabels = [], rootSchema) {
    ...
    if (interfaceOrTypeName && useLabels && !ignoreLabels.includes(interfaceOrTypeName)) {
        ...
        const child = types_1.makeTypeContentChild({
    ...
    if (!exports.supportedJoiTypes.includes(details.type)) {
        ...
        let typeToUse;
        const baseTypes = joiUtils_1.getMetadataFromDetails('baseType', details);
        ...
        return types_1.makeTypeContentChild({ content: typeToUse, interfaceOrTypeName, jsDoc });
    }
...
function parseBasicSchema(details, settings, rootSchema) {
    ...
    if (values && values.length !== 0) {
        ...
        return types_1.makeTypeContentRoot({ joinOperation: 'union', children: allowedValues, interfaceOrTypeName, jsDoc });
    }
    if (rootSchema) {
        return types_1.makeTypeContentRoot({
            joinOperation: 'union',
            children: [types_1.makeTypeContentChild({ content, interfaceOrTypeName, jsDoc })],
            interfaceOrTypeName,
            jsDoc
        });
    }
    else {
        return types_1.makeTypeContentChild({ content, interfaceOrTypeName, jsDoc });
    }
}
...
function parseStringSchema(details, settings, rootSchema) {
    ...
    if (rootSchema) {
        return types_1.makeTypeContentRoot({
            joinOperation: 'union',
            children: [types_1.makeTypeContentChild({ content: 'string', interfaceOrTypeName, jsDoc })],
            interfaceOrTypeName,
            jsDoc
        });
    }
    else {
        return types_1.makeTypeContentChild({ content: 'string', interfaceOrTypeName, jsDoc });
    }
}
function parseArray(details, settings) {
    ...
    // at least one value
    if (allowedValues.length !== 0) {
        allowedValues.unshift(types_1.makeTypeContentRoot({ joinOperation: 'list', children: [child], interfaceOrTypeName, jsDoc }));
        return types_1.makeTypeContentRoot({ joinOperation: 'union', children: allowedValues, interfaceOrTypeName, jsDoc });
    }
    return types_1.makeTypeContentRoot({ joinOperation: 'list', children: [child], interfaceOrTypeName, jsDoc });
}
function parseAlternatives(details, settings) {
    ...
    return types_1.makeTypeContentRoot({ joinOperation: 'union', children, interfaceOrTypeName, jsDoc });
}
function parseObjects(details, settings) {
    ...
    // at least one value
    if (allowedValues.length !== 0) {
        allowedValues.unshift(types_1.makeTypeContentRoot({ joinOperation: 'object', children, interfaceOrTypeName, jsDoc }));
        return types_1.makeTypeContentRoot({ joinOperation: 'union', children: allowedValues, interfaceOrTypeName, jsDoc });
    }
    return types_1.makeTypeContentRoot({ joinOperation: 'object', children, interfaceOrTypeName, jsDoc });
}
